...
from pomprl.rl.envs.stratego.stratego_spatial_multiagent_env import SpatialStrategoMultiAgentEnv

# Need raw numba functions
from pomprl.games.stratego.stratego_game_logic import _get_game_result_is_invalid

# Numba Methods
from pomprl.rl.envs.stratego.stratego_spatial_parallel_logic import _fill_observation, _step_multi_envs

from ray.tune.registry import register_env
from ray.tune.logger import pretty_print

from pomprl.games.stratego.stratego_game_logic import SP


SPATIAL_STRATEGO_PARALLEL_ENV = "spatial_stratego_parallel_env"


def build_heuristic_reward_matrix(reward_dict):
    # Builds a matrix for reward[as piece type, taking piece type]

    # UNKNOWN is the highest piece enum value (numbered 0-13) so it defines the shape of the matrix
    heuristic_reward_matrix = np.zeros(shape=(SP.UNKNOWN.value + 1, SP.UNKNOWN.value + 1), dtype=np.float32)
    for (as_piece, taking_piece), reward in reward_dict.items():
        heuristic_reward_matrix[as_piece.value, taking_piece.value] = reward
    return heuristic_reward_matrix
...
class StrategoParallelEnv(BaseEnv):
    def __init__(self, env_config: dict = None, num_envs: int = 1):
        ...
        if env_config.get('heuristic_reward_matrix') is not None:
            if isinstance(env_config['heuristic_reward_matrix'], str):
                # in this case it should be a path to a json file with the matrix
                json_path = env_config['heuristic_reward_matrix']
                converted_dict = {}
                with open(json_path) as json_file:
                    unconverted_dict = json.load(json_file)
                for key_str, reward in unconverted_dict.items():
                    attacking_piece_str, defending_piece_str = key_str.split(',')
                    attacking_piece_str = attacking_piece_str.replace("SP.", "").strip().upper()
                    defending_piece_str = defending_piece_str.replace("SP.", "").strip().upper()

                    converted_dict[(SP[attacking_piece_str], SP[defending_piece_str])] = float(reward)
                print(f"\n\n\n\nParsed the following rewards dict from {json_path}:\n{converted_dict}\n\n\n\n")
                env_config['heuristic_reward_matrix'] = build_heuristic_reward_matrix(reward_dict=converted_dict)
    ...
    def poll(self):
        ...
        for env_idx, (player, obs, reward, done, state) in enumerate(
            ...
            if not done:
                ...
                if self.penalize_ties and env_info[1]['game_result'] == 'tied':
                    rewards[env_idx] = {player: -0.5, -player: -0.5}

                info[env_idx] = env_info

            # self.envs[env_idx].base_env.print_fully_observable_board_to_console(state=self.state_buffer[env_idx],
            #                                                                     hide_still_piece_markers=True)
            # print(f"player: {player}, rewards: {rewards}, done: {dones}")
    ...
    def send_actions(self, action_dict):
        ...
        for i, ac in action_dict.items():
            action = ac[self.player_buffer[i]]
            action = np.unravel_index(action, shape=self.base.base_env.spatial_action_size)
            action = self.base.base_env.get_action_1d_index_from_spatial_index(action)
            actions[i] = action
...
register_env(SPATIAL_STRATEGO_PARALLEL_ENV, make_stratego_parallel_env)
...
if __name__ == '__main__':
    ...
    old_env = BaseEnv.to_base_env(SpatialStrategoMultiAgentEnv(), lambda x: SpatialStrategoMultiAgentEnv(), num_envs=num_envs)
    old_env.poll()
    env.poll()
