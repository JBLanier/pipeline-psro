...
from ray.rllib.evaluation.rollout_worker import _validate_multiagent_config

from pomprl.rl.common.shared_pol_rollout_worker import SharedPolicyRolloutWorker
...
@DeveloperAPI
class SharedPolicyRolloutWorkerSet(object):
    ...
    def __init__(self,
        ...
        if _setup:
            ...
            self._local_worker = self._make_worker(
                SharedPolicyRolloutWorker, env_creator, policy, 0, self._local_config,
                local_shared_policy_map=local_shared_policy_map, local_shared_preprocessors=local_shared_preprocessors,
    ...
    def add_workers(self, num_workers):
        ...
        cls = SharedPolicyRolloutWorker.as_remote(**remote_args).remote
        # jb changed this ^
    ...
    def _from_existing(local_worker, remote_workers=None):
        workers = SharedPolicyRolloutWorkerSet(None, None, {}, _setup=False)
    ...
    def _make_worker(self, cls, env_creator, policy, worker_index, config,
        ...
        if local_shared_tf_sess:
            session_creator = lambda: local_shared_tf_sess
        elif config["tf_session_args"]:
            def default_session_creator():
                logger.debug("Creating TF session {}".format(
                    config["tf_session_args"]))
                return tf.Session(
                    config=tf.ConfigProto(**config["tf_session_args"]))
            session_creator = default_session_creator
        # jb changed this ^
