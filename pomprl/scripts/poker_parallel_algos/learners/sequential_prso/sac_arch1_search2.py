...
MANAGER_SEVER_HOST = os.getenv("MANAGER_SERVER_HOST", "sage")
...
if __name__ == "__main__":
    ...
    def all_on_train_result_callbacks_no_sampling(params):
        stop_and_submit_if_not_improving_on_train_result_callback(params=params)

    def all_on_train_result_callbacks_with_sampling(params):
        stop_and_submit_if_not_improving_on_train_result_callback(params=params)
    ...
    "callbacks": tune.grid_search([
        {
            "on_train_result": all_on_train_result_callbacks_with_sampling,
            # 'on_episode_start': sample_new_static_policy_weights_for_each_worker_on_episode_start,
        },
        {
            "on_train_result": all_on_train_result_callbacks_no_sampling,
            'on_episode_start': sample_new_static_policy_weights_for_each_worker_on_episode_start,
        }
    ]),
    "buffer_size": 1000000,
    ...
    def trial_name_creator(trial):
        config = trial.config
        return f"{random_string(2)}_nmwrkrs_{config['num_workers']}_" \
               f"nmenvs_{config['env_config']['num_envs']}_callback_{config['callbacks']['on_train_result'].__name__}"
