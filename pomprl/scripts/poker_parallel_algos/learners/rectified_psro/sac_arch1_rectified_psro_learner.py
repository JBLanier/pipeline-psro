...
from population_server.lock_server.lock_client_interface import LockServerInterface
...
from pomprl.scripts.population_server.utils.metanash import get_rectified_selection_probs_for_policy_key
...
if __name__ == "__main__":
    ...
    while True:
        ...
        active_job = None
        job_was_taken = False
        while not job_was_taken:
            all_jobs = lock_server_interface.get_all_items(filter_by_string="job: ")
            available_jobs = [job for job in all_jobs if 'open' in job]
            if len(available_jobs) > 0:
                for available_job in available_jobs:
                    new_job_key = available_job.replace("open", "active")
                    job_was_taken = lock_server_interface.replace_item(old_item=available_job,
                                                                       new_item=new_job_key,
                                                                       new_item_remains_after_disconnect=True)
                    if job_was_taken:
                        active_job = new_job_key
                        break
            print("waiting for available jobs")
            time.sleep(10)

        _, job_init_policy_key, _ = active_job.split(' ')
        print(colored(f"job init policy is {job_init_policy_key}", "yellow"))
        assert isinstance(job_init_policy_key, str)

        del lock_server_interface  # ray tries to pickle this an cant, fun errors occur

        ray.init(address=os.getenv('RAY_HEAD_NODE'), ignore_reinit_error=True)
        logger.info("Ray Web UI at {}".format(ray.get_webui_url()))

        base_experiment_name = f"{CLOUD_PREFIX}learner_{POKER_GAME_VERSION}_sac_arch1_psro_rectified"
        ...
        # active_job = "job: random active"
        # job_init_policy_key = "random"
        ...
        def stop_and_submit_if_not_improving_on_train_result_callback(params):
            ...
            if should_submit:
                ...
                job_submitted_key = active_job.replace('active', policy_key)
                assert trainer.lock_server_interface.replace_item(old_item=active_job,
                                                           new_item=job_submitted_key,
                                                           new_item_remains_after_disconnect=True)
