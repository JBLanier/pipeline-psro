...
from population_server.utils import ensure_dir
...
def move_object(minio_client: Minio, bucket_name, source_object_name, dest_object_name):
    assert copy_object(minio_client=minio_client, bucket_name=bucket_name, source_object_name=source_object_name,
                       dest_object_name=dest_object_name)
    minio_client.remove_object(bucket_name=bucket_name, object_name=source_object_name)
...
def maybe_download_objects_with_prefix(minio_client: Minio, bucket_name, object_prefix,
    ...
    for obj in objects:
        if not obj.is_dir:
            file_path, info = maybe_download_object(minio_client=minio_client, bucket_name=bucket_name,
...
def get_n_evenly_spaced_policy_keys_with_prefix_sorted_by_iter(minio_client: Minio, n: int, bucket_name: str,
                                                               object_prefix: str,
                                                               recurse_into_dirs: bool, iter_num_regex="(?<=iter_)\d+"):
...
def post_key(minio_client, bucket_name, key, bulletin_prefix="bulletin"):
    ...
    with open(post_local_path, "+w") as file:
        file.write(key)
    return upload_file(minio_client=minio_client,
                bucket_name=bucket_name,
                object_key=new_key,
                local_source_path=post_local_path)
