...
from population_server.cloud_storage import connect_minio_client, upload_file, get_default_path_on_disk_for_minio_key, post_key
from population_server.cloud_storage import maybe_download_object
from population_server.payoff_table import PayoffTable, PolicySpec
from population_server.protobuf.population_server_pb2 import WorkerPing, PayoffTableKey, PolicyInfo, Confirmation, \
    EvalMatchupOrder, EvalMatchupResult, ManagerStats
from population_server.protobuf.population_server_pb2_grpc import PopulationServerServicer, \
    add_PopulationServerServicer_to_server
from population_server.utils import datetime_str, ensure_dir, seconds_to_text
...
class _PopulationServerServicerImpl(PopulationServerServicer):
    ...
    def __init__(self,
        ...
        logger.info(colored(f"Latest Checkpoint will always be at {self._latest_checkpoint_key}", "yellow"))
...
class ManagerServer(object):
    ...
    def __init__(self, config):
        ...
        self._minio_client = connect_minio_client(endpoint=MINIO_ENDPOINT,
