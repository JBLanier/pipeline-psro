from population_server.real_time_payoff_tracker.protobuf.real_time_payoff_tracker_pb2 import PayoffTableKey, PolicyInfo, LivePolicyInfo, Confirmation, EvalMatchupResult
from population_server.real_time_payoff_tracker.protobuf.real_time_payoff_tracker_pb2_grpc import RealTimePayoffTrackerServicer, \
    add_RealTimePayoffTrackerServicer_to_server
...
from population_server.cloud_storage import connect_minio_client, MINIO_DEFAULT_SAVE_PATH, get_default_path_on_disk_for_minio_key, upload_file, post_key
from population_server.utils import datetime_str, pretty_print, ensure_dir
from population_server.worker.console import ConsoleManagerInterface
import dill

WORKER_ID = f"real_time_payoff_tracker_{gethostname()}_pid_{os.getpid()}_{datetime_str()}"
...
LATEST_CHECKPOINT_KEY = f"{CLOUD_PREFIX}real_time_payoff_tracker_checkpoints/{WORKER_ID}/latest.dill"
...
class RealTimePayoffTrackerServicerImpl(RealTimePayoffTrackerServicer):

    def __init__(self, minio_client, bucket_name):

        self._current_live_payoff_table = None
        self._latest_live_payoff_table_key = None
        self._payoff_modification_lock = threading.RLock()
    ...
    def GetLatestLivePayoffTableKey(self, request, context):
        latest_payoff_table_key = self._latest_live_payoff_table_key
    ...
    def _save_checkpoint(self, checkpoint_name):
        ...
        upload_file(minio_client=self._minio_client,
        ...
        if CLOUD_PREFIX:
            post_key(minio_client=self._minio_client,
                     bucket_name=self._bucket_name,
...
if __name__ == '__main__':
    ...
    logger.info(colored(f"Latest checkpoint will always be at {LATEST_CHECKPOINT_KEY}", 'yellow'))
    ...
    lock_servicer = RealTimePayoffTrackerServicerImpl(minio_client=minio_client, bucket_name=BUCKET_NAME)
    add_RealTimePayoffTrackerServicer_to_server(servicer=lock_servicer, server=grpc_server)
    grpc_server.add_insecure_port(f"[::]:{config['real_time_payoff_tracker_grpc_port']}")

    logger.info(f"Starting GRPC server. Listening on port {config['real_time_payoff_tracker_grpc_port']}.")
