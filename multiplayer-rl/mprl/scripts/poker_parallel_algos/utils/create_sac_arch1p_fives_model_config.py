...
from po.scripts.population_server.utils.policy_config_keys import SAC_ARCH1P_FIVES_MODEL_CONFIG_KEY
...
if __name__ == "__main__":
    ...
    base_experiment_name = f"create_{STRATEGO_GAME_VERSION}_sac_arch1p_fives_policy_config"
    ...
    if key_exists(storage_client=storage_client, bucket_name=BUCKET_NAME, object_name=SAC_ARCH1P_FIVES_MODEL_CONFIG_KEY):
        response = input(f"\n\nThe key {SAC_ARCH1P_FIVES_MODEL_CONFIG_KEY} already exists. "
        ...
        if response != 'y':
            print("exiting...")
            exit(0)
        response = input(f"Are you REALLY sure you want to overwrite {SAC_ARCH1P_FIVES_MODEL_CONFIG_KEY}?????"
    ...
    upload_file(storage_client=storage_client,
                ...
                object_key=SAC_ARCH1P_FIVES_MODEL_CONFIG_KEY,
