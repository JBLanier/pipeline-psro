...
import logging
from mprl.rl.envs.stratego.stratego_multiagent_env import STRATEGO_ENV
from mprl.utility_services.cloud_storage import upload_file, maybe_download_object, get_default_path_on_disk_for_minio_key
...
def save_model_weights_to_disk_tmp_and_minio(storage_client, bucket_name, policy_to_save, policy_key, new_object_key):
    ...
    upload_file(storage_client, bucket_name=bucket_name, object_key=new_object_key, local_source_path=disk_path)


def load_model_weights_from_disk_tmp_or_minio(storage_client, bucket_name, policy_to_set, policy_key, object_key):
    maybe_download_object(storage_client, bucket_name=bucket_name, object_name=object_key)
