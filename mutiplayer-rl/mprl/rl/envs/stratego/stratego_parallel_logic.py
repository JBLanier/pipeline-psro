...
from stratego_env.game.stratego_procedural_impl import _get_state_from_player_perspective, _get_valid_moves_as_1d_mask, \
    _get_partially_observable_observation, _get_fully_observable_observation, \
    _get_action_1d_index_from_player_perspective, _get_next_state, _get_game_ended, \
    _get_heuristic_rewards_from_move
...
@jit(f"void(int64, int64, int64, int64, int64, "
     f"float32[:, :, ::1], float32[:, :, ::1], float32[:, :, ::1], float32[:, :, ::1], boolean, boolean, boolean,"
     f"{STSH}, int64[::1], {OTSH}, {OTSH}, {OTSH})", nopython=True, cache=True)
def _fill_observation(player, action_size, max_actions, rows, columns,
                      p_obs_mids, p_ops_range, f_obs_mids, f_obs_range, partial_out, full_out, internal_out,
    ...
    valid_actions_mask[:] = _get_valid_moves_as_1d_mask(state=pstate, player=np.int64(1), action_size=action_size,
...
@jit(f"float32(int64, int64, int64, int64, int64, int64, "
     f"float32[:, :, ::1], float32[:, :, ::1], float32[:, :, ::1], float32[:, :, ::1], boolean, boolean, boolean,"
     f"{STSH}, int64[::1], {OTSH}, {OTSH}, {OTSH},"
...
def _step_single_env(player, action, action_size, max_actions, rows, columns,
    ...
    _fill_observation(player, action_size, max_actions, rows, columns,
                      ...
                      partial_out, full_out, internal_out,
                      state, valid_actions_mask, partial_observation,
                      full_observation, internal_state)

    reward = _get_game_ended(state=state, player=player) + heuristic_reward
...
@jit(f"void(int64[::1], int64[::1], float32[::1], boolean[::1], int64, int64, int64, int64, "
     f"float32[:, :, ::1], float32[:, :, ::1], float32[:, :, ::1], float32[:, :, ::1], boolean, boolean, boolean,"
     f"{MSTSH}, int64[:, :, ::1], {MOTSH}, {MOTSH}, {MOTSH},"
...
def _step_multi_envs(players, actions, reward_buffer, done_buffer, action_size, max_actions, rows, columns,
    ...
    for i in prange(num_envs):
        ...
        reward = _step_single_env(player, action, action_size, max_actions, rows, columns,
                                  p_obs_mids, p_ops_range, f_obs_mids, f_obs_range,
                                  partial_out, full_out, internal_out,
                                  state, valid_actions_mask, partial_observation, full_observation, internal_state,
                                  add_heuristic_rewards, heuristic_reward_matrix)
